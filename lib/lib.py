...
def search_links(stem: str) -> dict:
    """
    ...
    new_urls = check_new(listing_dict)
    return(new_urls)

def count_title_emoji(soup) -> int:
    """
...
def parse_posting_info(soup) -> tuple:
    """
...
def metrics_from_soup(soup) -> list:
    """
...
def metrics_from_text(body: str) -> list:
    """
    This function returns a number of metrics from the post text
...
def get_sqft(soup, body: str) -> str:
    """
...
def dog_friendliness(soup, body: str) -> str:
    """
...
def make_output(soup_metrics, dog, sqft, text_metrics, address, snippet, url):
    """
    ...
    address, zipcode, neighborhood, locality = address
...
def extract_soup(soup, url):
    ...
    with open(os.path.join(outdir, soup_metrics[2] + '.txt'), "w") as file:
        file.write(posttext)
    # get first 100 characters of post body
    snippet = posttext[1:100]
    ...
    json = reverse_lookup(api_key, soup_metrics[0], soup_metrics[1])
